defmodule Openmaize.Signup do
  ...
  * common - check to see if the password is too common (easy to guess)
  ...
  In the following example, a key and link are generated by the `gen_token_link`
  function, and then the `add_confirm_token` function is called:

      {key, link} = Openmaize.Signup.gen_token_link(email)
      changeset
